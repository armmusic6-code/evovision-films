...
export async function fetchAllContent(): Promise<ContentItem[]> {
  try {
    ...
    return data.map((row: Record<string, unknown>) => mapContentRow(row));
  } catch {
    return [];
  }
}
...
export async function fetchAllSponsors(): Promise<Sponsor[]> {
  try {
    ...
    return data.map((row: Record<string, unknown>) => mapSponsorRow(row));
  } catch {
    return [];
  }
}

function mapSponsorRow(row: Record<string, unknown>): Sponsor {
